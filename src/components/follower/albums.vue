  ...
  <div>
        ...
        <div @click="openAlbum" class="overlay">
          <div>
            <span class="text-hover">
              <div>{{ $t("follower.Album_Name") }}</div>
              <span> {{images.length}} {{ $t("follower.item_s") }}</span>
            </span>
          </div>
        </div>
      </div>
    ...
    <div v-if="pictureShow">
      ...
      <div class="album-detail">
        <div class="album-name"><b>{{ $t("follower.Album_Name") }}</b></div>
        <span> {{ images.length }} {{ $t("follower.Pictures") }} </span>
      </div>
      <div class="albums">
        ...
        <b-modal hide-footer v-model="modalShow">
          ...
          ></b-img>
          <b-row>
            <h3 class="ml-3">{{ $t("follower.Picture_Title") }}</h3>
            <b-icon
              icon="download"
