...
export default {
  ...
  actions: {
    ...
    async updateUserBasicInfosMobilePhones(context, payload) {
      ...
      const lastPhoneNumber = payload.mobilePhones[payload.mobilePhones.length - 1];
      let response_ = null;
      await axios({
        method: 'post',
    ...
    async updateUserBasicInfosWebsites(context, payload) {
      ...
      let response_ = null;
      await axios
        .post('/userIntro/storeWebLink' + '?webUrl=' + payload.websites[payload.websites.length - 1], {
          method: 'POST',
          headers: {
            Accept: 'application/json',
          },
    ...
    async updateUserEducation(context, payload) {
      ...
      if (payload.method.toLowerCase() === 'post') {
        ...
        (url = 'userIntro/updateSchool' + '/11' + '?schoolName=' + payload.education.schoolName),
          '&graduated=' + graduated,
          '&durationFrom=' + payload.education.durationFrom,
          '&major=' + payload.education.major,
          '&durationTo=' + payload.education.durationFrom;
