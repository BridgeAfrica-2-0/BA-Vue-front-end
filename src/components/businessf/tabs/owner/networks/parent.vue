<template>
	<div class="">
		<b-card title="" class="">
			...
			<b-container v-else class="a-center">
				<!-- :src="require('@/assets/img/mayor.jpg')" -->
				<!-- :src="networkInfo[0].image" -->
        <!-- :src="require('@/assets/img/mayor.jpg')" -->
				<b-avatar
					variant="primary"
          :src="require(networkInfo.image)" 
					square
					rounded
					class="network-logo"
				>
      ...
      this.isLoading = true
		...
		methods: {
			init: async function() {
				...
				const idNetWork = this.$route.params.id;
				await this.getNetworkInfo(idNetWork);
			},
			openNetwork() {
				this.networkShow = false;
			},
			asignUpdateForm(network) {
				this.updateNetwork_form = {
					name: network.name,
					description: network.description,
					email: network.email,
					primary_phone: network.primary_phone,
					secondary_phone: network.secondary_phone,
					address: network.address,
					allow_business: network.allow_business,
				};
				console.log(this.updateNetwork_form);
			},
			editNetwork() {
				this.asignUpdateForm(this.networkInfo);
			...
			getNetworkInfo(idNetWork) {
				this.$store
					.dispatch("networkProfile/getnetworkInfo", idNetWork)
					.then(() => {
						this.asignUpdateForm(this.networkInfo);
