    ...
    <div v-for="network in networks" :key="network.id"  class="people-style shadow">
      <b-row>
        ...
        <b-col md="5" cols="7" lg="7" xl="7" sm="5">
          <p class="textt">
            <strong class="title"> {{network.name.substring(0,15)+"..." }} </strong> <br />
            {{network.category != 0 ? network.category[0].name : "Null"}}
            <br />
            ...
            <br />
            <span v-if="network.description.length<65">{{ network.description}}</span>
            <span v-else >{{ network.description.substring(0,65)+"..." }} <b-link>Read More</b-link></span>
          </p>
        </b-col>
