...
class Repository {
  ...
  async findPostByKeyword(credentials) {
    try {
      ...
      const response = await axios.post(
        `search/people/post/${page}?keyword=${data.keyword}`,
        { ...data, page }
      );
      return {
        success: response.data.data ? true : false,
  ...
  async findPostForGuestUser(credentials) {
    try {
      ...
      const response = await axios.get(
        `visitor/search/post?limit=10&page=${page}&keyword=${data.keyword}`
      );
      return {
        success: response.data.data ? true : false,
        data: response.data.data ? response.data.data : []
  ...
  async findPostByBuisness(credentials) {
    try {
      const { page, data } = credentials;
      const response = await axios.post(`search/business/post/${page}`, {
        ...data,
        page
      });
      return {
  ...
  async findPostByBusinessForGuestUser(credentials) {
    try {
      const { page, data } = credentials;
      const response = await axios.post(
        `visitor/search/business/post/${page}`,
        { ...data, page }
      );
      return {
        success: response.data.data ? true : false,
  ...
  async findPostByNetWork(credentials) {
    try {
      ...
      const response = await axios.post(
        `search/network/post/${keyword}/${page}`,
        { ...data, page }
      );
      return {
        success: response.data.data ? true : false,
