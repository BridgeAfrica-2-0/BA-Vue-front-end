...
export const mutations = {
  [TYPES.FIND_USER](state, payload) {
    if (state.page > 1) state.users = [...state.users, ...payload];
    else state.users = payload;
  }, //[TYPES.FIND_USER]

  [TYPES.FIND_POST](state, payload) {
    if (state.page > 1) state.posts = [...state.posts, ...payload];
  ...
  [TYPES.LOADING](state, payload) {
    ...
    else throw new Error('payload must be a bolean');
  }, // [TYPES.LOADING]

  [TYPES.SET_CURRENT_PAGINATION_PAGE](state, payload) {
    if (typeof payload === 'number') state.page = payload;
